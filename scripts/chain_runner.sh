...
eval $NEXT
export LR=$lr EPOCHS=$epochs NAME=$name BS=${bs:-8} OPTIMIZER=${optimizer:-AdamW} WARMUP=${warmup:-1000} WD=${wd:-0.05} IMGSZ=${imgsz:-640} MODEL=${model:-yolov8l-worldv2.pt} FREEZE=${freeze:-0} MOSAIC=${mosaic:-1.0} AUTO_AUGMENT=${auto_augment:-randaugment} ERASING=${erasing:-0.4} SCALE=${scale:-0.5}
