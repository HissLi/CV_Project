#!/bin/bash
set -euo pipefail
#SBATCH --job-name=gdino_refcoco_eval
#SBATCH --partition=a100
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=40G
#SBATCH --time=24:00:00
#SBATCH --output=/home/turing_lab/cse12210210/cv_project/logs/%j_gdino_refeval.out
#SBATCH --error=/home/turing_lab/cse12210210/cv_project/logs/%j_gdino_refeval.err
...
echo "DATASETS=${DATASETS:-refcoco,refcoco+,refcocog} SPLIT=${SPLIT:-val} MAX_SAMPLES=${MAX_SAMPLES:-all} THRESHOLD=${THRESHOLD:-0.25}"
...
python ~/cv_project/scripts/gdino/eval_refcoco.py \
  --datasets "${DATASETS:-refcoco,refcoco+,refcocog}" \
  --split "${SPLIT:-val}" \
  --model_dir "${MODEL_DIR:-~/cv_project/models/gdino}" \
  ...
  --output_dir "${OUTPUT_DIR:-~/cv_project/results/gdino_refcoco_zeroshot}" \
